...
`--auto-approve` is set by the dispatcher when the owning workflow has the per-workflow auto-approve-gates toggle on (#2349). It bypasses the §8 human gate only — every other step (skip rules, cap, file-only boundary, the §6 reconcile: fine-key deduplicate-and-close plus coarse-key umbrella-append) is unchanged. Absent the flag, the gate stays in force.
...
- **Never creates an epic. Creates an umbrella only for a mechanical sweep at threshold (§6c, #2517).** The skill never passes `--epic` and never creates an epic issue. It creates **at most one umbrella issue** per coarse key, and **only** through §6c's threshold-escalation path: a `new_helper` / `new_pattern` / `perf_fix` (mechanical-sweep) key whose running site count crosses the threshold (>5 sites, or recurs across >1 scan run). **§6c is the single place in this skill that mints an umbrella** — no other section, and never for a `bugfix_shape` key (those stay individual per-site issues, for individual review). Below threshold, and for every `bugfix_shape` key, the skill files **flat** `child-of` per-site children of the scan target and creates nothing. This is the scoped relaxation of the pre-#2517 absolute "never creates an umbrella" boundary: the relaxation is *owned here* and is exactly the mechanical-sweep-at-threshold case — nothing wider. The §6b accumulator still only **appends to an umbrella that already exists**; it does not create one.
