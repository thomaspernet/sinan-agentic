...
### 6c. Threshold escalation — create an umbrella for an eligible key
...
Check both with one read, then open the section the marker hits and confirm (2). Anchor the
pattern — an unanchored match also hits a *longer* key that carries yours as a prefix, which
would graduate a key on another key's rule:
...
Escalation (§6c): promoted <E> eligible key(s) to umbrella #<U3> (<G> site(s)); folded #<F1>, #<F2> → #<U3>.
